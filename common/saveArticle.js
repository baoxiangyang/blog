			...
			let	promiseArr = [], $, dataObject, container;
				...
				if(/^\/a\//.test(url)){
					dataObject = {
						...
						time: returnTime($('.article__authorright span').eq(2).text().trim()),
				...
				if(promiseArr.length){
					Promise.all(promiseArr).then((data)=>{
						...
						fs.writeFile(htmlSavePath + '/'+ dataObject.id +'.html', container.html().trim().trim(), function(err){
							if(err){
								reject({
									err: err,
									url: url
...
function returnLabel($){
	let arr = [];
	$('.taglist--inline li').each(function(){
		arr.push($(this).find('a').text().trim());
	});
	return arr;
}
